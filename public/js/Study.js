document.addEventListener('DOMContentLoaded', function() {
  var Studysave = JSON.parse(localStorage.getItem('Studysave')) || [];
  var listContainer = document.getElementById('list-container');

  // 초기 목록 출력
  Studysave.forEach(function(post, index) {
  ...
  function submitPost() {
    ...
    // 글 목록에 추가
    Studysave.push(post);
    localStorage.setItem('Studysave', JSON.stringify(Studysave));

    // 목록에 새 글 추가
    var listItem = createListItem(post, Studysave.length - 1);
    listContainer.appendChild(listItem);
